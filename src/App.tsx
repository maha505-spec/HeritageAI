...
import { analyzeManuscript, ManuscriptAnalysis } from './services/geminiService';
...
const Login = ({ onLogin }: { onLogin: (user: LocalUser) => void }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    localStorage.setItem('heritage_user', JSON.stringify(mockUser));
    onLogin(mockUser);
  };
  ...
  return (
    <div className="min-h-screen bg-[#FDFCF8] flex items-center justify-center p-4">
      <motion.div 
        ...
        className="max-w-md w-full bg-white p-8 rounded-3xl shadow-xl border border-emerald-100"
      >
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500">Ancient Manuscript Digitalization System</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button 
            type="submit"
            className="w-full bg-emerald-600 text-white py-4 rounded-xl font-semibold hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-200 mt-4"
          >
            Access Archive
          </button>
        </form>
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        const savedManuscripts = JSON.parse(localStorage.getItem('heritage_manuscripts') || '[]');
        localStorage.setItem('heritage_manuscripts', JSON.stringify([newManuscript, ...savedManuscripts]));

        // Trigger a re-load of manuscripts
...
export default function App() {
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem('heritage_user');
    if (savedUser) {
  ...
  useEffect(() => {
    const loadManuscripts = () => {
      ...
      const allManuscripts = JSON.parse(localStorage.getItem('heritage_manuscripts') || '[]');
      const userManuscripts = allManuscripts.filter((m: any) => m.userId === user.uid);
      setManuscripts(userManuscripts);
    };

    loadManuscripts();

    // Listen for local storage changes (for same-tab updates)
    window.addEventListener('storage', loadManuscripts);
    return () => window.removeEventListener('storage', loadManuscripts);
  }, [user]);
